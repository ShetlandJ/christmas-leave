...
- Local: `/seed [density]` builds `dev/index.html` (real UI, in-page fetch stub, fake names, leave booked in realistic blocks) and serves it on http://localhost:8731. `#local-1` … `#local-22` act as each fake person. `/unseed` blanks it. Nothing in `dev/` touches Google.
